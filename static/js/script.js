...
function startTelegramLinkMonitoring(telegramCode, nextStepUrl) {
    ...
    showNotification('🔍 جاري مراقبة حالة الربط...', 'info');
...
// ⏰ خيارات في حالة انتهاء الوقت - محدثة
function showTelegramTimeoutOptions(telegramCode, nextStepUrl) {
    // إظهار خيارات للمستخدم
    const userChoice = confirm(`⏰ انتهت مهلة انتظار الربط التلقائي.

هل تريد:
✅ الانتقال لصفحة الكوينز (اضغط موافق)
🔄 المحاولة مرة أخرى (اضغط إلغاء)

ملاحظة: يمكنك إكمال الربط لاحقاً من التليجرام`);
    
    if (userChoice) {
        // الانتقال المباشر لصفحة الكوينز
        ...
        // إعادة المحاولة
        showNotification('🔄 إعادة بدء مراقبة الربط...', 'info');
...
// دالة ربط التليجرام - إضافة جديدة كاملة
async function linkTelegram() {
    ...
    if (!statusDiv) {
        ...
        // البحث عن مكان مناسب لوضع الحالة
    ...
    if (!whatsappNumber) {
        showStatus('يرجى إدخال رقم الواتساب', 'error');
        whatsappInput.focus();
        return;
    }
    
    if (!telegramCode) {
        showStatus('يرجى إدخال كود التليجرام', 'error');
    ...
    if (!whatsappRegex.test(cleanWhatsapp)) {
        showStatus('رقم الواتساب غير صحيح. يجب أن يبدأ بـ 010 أو 011 أو 012 أو 015', 'error');
        whatsappInput.focus();
        return;
    }
    ...
    if (submitBtn) {
        submitBtn.disabled = true;
        submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> جاري الربط...';
    }
    ...
    try {
        ...
        if (data.success) {
            ...
            // الانتقال إلى صفحة الكوينز بعد 2 ثانية
            setTimeout(() => {
                if (data.redirect) {
        ...
        if (submitBtn) {
            ...
            submitBtn.innerHTML = '<i class="fas fa-link"></i> ربط الحساب';
        }
    }
}
