 ...
 * 
 * @version 2.3.0 - FIXED AUTO-START VERSION
 * @author FC26 Team
 * @description صندوق أسود لكل ما يخص ربط التليجرام - الإصدار النهائي المحسّن مع إصلاح undefined
 */
    ...
    try {
        ...
        if (serverResponse.success && serverResponse.telegram_code) {
            console.log('🔗 نجح الحصول على بيانات التليجرام:', {
                ...
                code: serverResponse.telegram_code ? serverResponse.telegram_code.substring(0, 10) + '...' : 'NO_CODE'
            });
            
            // إنشاء بيانات محسنة للتليجرام مع التأكد من وجود الكود
            const enhancedData = {
                ...serverResponse,
                bot_username: serverResponse.bot_username || 'ea_fc_fifa_bot',
                telegram_code: serverResponse.telegram_code,
                telegram_web_url: `https://t.me/${serverResponse.bot_username || 'ea_fc_fifa_bot'}?start=${serverResponse.telegram_code}`,
                telegram_app_url: `tg://resolve?domain=${serverResponse.bot_username || 'ea_fc_fifa_bot'}&start=${serverResponse.telegram_code}`
            };
            
            // فتح التليجرام بالطريقة الذكية المحسّنة
            await openTelegramSmartly(enhancedData);
            
            // عرض الكود للنسخ اليدوي
            displayCopyableCode(telegramBtn, enhancedData);
            
            // بدء مراقبة الربط
            if (enhancedData.telegram_code) {
                startTelegramLinkingMonitor(enhancedData.telegram_code);
            }
...
async function sendTelegramLinkRequest(formData) {
    ...
    try {
        ...
        console.log('📦 محتوى الاستجابة:', {
            ...
            code: result.telegram_code ? result.telegram_code.substring(0, 10) + '...' : 'NO_CODE',
...
/**
 * 📱 فتح التليجرام بالطريقة الذكية - ENHANCED للـ auto-start مع إصلاح undefined
 */
async function openTelegramSmartly(data) {
    ...
    if (!telegramCode) {
        console.error('❌ لا يوجد كود تليجرام!');
        showTelegramNotification('❌ خطأ: لم يتم الحصول على كود التليجرام', 'error');
        return;
    }
    
    // 🔥 إنشاء روابط محسّنة للتفعيل التلقائي مع التأكد من الكود
...
console.log('📦 Telegram Integration Module v2.3.0 - FINAL FIXED - تم التحميل بنجاح');
